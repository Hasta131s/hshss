...
object PlaybackManager {
    ...
    private val _alignedLyrics = MutableStateFlow<List<com.example.smartlyrics.WordTiming>>(emptyList())
    val alignedLyrics = _alignedLyrics.asStateFlow()

    private val _lyricsState = MutableStateFlow(LyricsState.IDLE)
    val lyricsState = _lyricsState.asStateFlow()

    private fun alignLyricsAfterPrepared() {
        val track = _currentTrack.value ?: return
        val lyrics = _currentTrackLyrics.value ?: return
        val duration = _durationMs.value.toLong()
        if (duration <= 0L) return

        if (lyrics != "Sözler bulunamadı." && lyrics != "Enstrümantal veya Sözler henüz eklenmemiş.") {
            scope.launch {
                val wordTimings = com.example.smartlyrics.AILyricsEngine.alignAudioAndText(track.id, lyrics, duration)
                _alignedLyrics.value = wordTimings
            }
        }
    }
    ...
    fun loadLyricsForCurrentTrack() {
        ...
        scope.launch {
            try {
                ...
                // Align using AI Engine if duration is already known
                if (_durationMs.value > 100) {
                    alignLyricsAfterPrepared()
                } else {
                    _alignedLyrics.value = emptyList()
                }
            } catch (e: Exception) {
        ...
        scope.launch {
            ...
            try {
                ...
                player.setOnPreparedListener { mp ->
                    ...
                    if (_lyricsState.value == LyricsState.SUCCESS && !_currentTrackLyrics.value.isNullOrBlank()) {
                        alignLyricsAfterPrepared()
                    }
                }
